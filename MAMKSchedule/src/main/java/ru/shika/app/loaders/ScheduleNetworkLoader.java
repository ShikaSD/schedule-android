...
public class ScheduleNetworkLoader extends NetworkLoader {
    ...
    protected void prepareDownload() {
        if (!isNetworkConnection()) error(getString(R.string.error_network_not_connected));

        if (!isPersonalSchedule) query = prepareQuery();
        else
    ...
    private ParseQuery<ParseObject> prepareQuery() {
        ...
        if (items[0].group != null) query.whereEqualTo("group", items[0].group);
        else if (items[0].teacher != null) query.whereEqualTo("teacher", items[0].teacher);
    ...
    protected String insertValues(String type) {
        ...
        for (ParseObject i : result) {
            ...
            Cursor x = dbh.rawQuery("select * from Schedule where lesson = ? and teacher = ? and groups = ? and date = ? and start = ?", new String[]{i.getString("name"), i.getString("teacher"), i.getString("group"), i.getString("dateString"), i.getString("startTimeString")});
            //check for equality
            if (x.moveToFirst()) {
                dbh.update("Schedule", cv, "lesson = ? and teacher = ? and groups = ? and date = ? and start = ?", new String[]{i.getString("name"), i.getString("teacher"), i.getString("group"), i.getString("dateString"), i.getString("startTimeString")});
                x.close();
                continue;
            }
        ...
        try {
            ...
            for (ParseObject object : result) {
                ...
                do {
                    ...
                    if (object.getString("courseId").equals(c.getString(courseId)) || (object.getString("courseId").equals("") && object.getString("name").equals(c.getString(name)))) {
                        f = true;
                        break;
                    }
                } while (c.moveToNext());
