    ...
    protected void cursorParse(Cursor c) {
        ...
        if (c.moveToFirst()) {
            ...
            do {
                ...
                if (id == -1 || c.getString(id).equals("")) courseId = c.getString(name);
                else courseId = c.getString(id);

                if (courseId.equals("")) continue;
                ...
                for (String i : names.get(index)) {
                    if (i.replaceAll("[ ,.:;\\\\/-]+", "").toLowerCase().equals(c.getString(name).replaceAll("[ ,.:;\\\\/-]+", "").toLowerCase()) || (names.get(index).size() > 1 && c.getString(name).equals(courseId))) {
                        flag = true;
                        break;
                    }
                }
        ...
        if (checks.size() > 0) objects.add(checks);
