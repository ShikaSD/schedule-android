...
public class ListFragmentAdapter extends RecyclerView.Adapter<ListFragmentAdapter.ViewHolder> {
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(View itemView) {
            ...
            if (isCheckingList) check = (CheckBox) layout.findViewById(R.id.list_checkbox);
        }
    }
    ...
    public void onBindViewHolder(ViewHolder viewHolder, int i) {
        if (visible.size() > i && !visible.get(i)) viewHolder.layout.setLayoutParams(invis);
        else viewHolder.layout.setLayoutParams(vis);
        ...
        if (!keys.get(i).equals(names.get(i).get(0))) viewHolder.courseId.setText(keys.get(i));
        ...
        if (isCheckingList) {
            ...
            } else viewHolder.check.setVisibility(View.GONE);
        }
    }
    ...
    public void toggle(int key) {
        if (key < checkedItems.size()) checkedItems.set(key, !checkedItems.get(key));
        else {
    ...
    public void check(SparseBooleanArray checks) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (key < checkedItems.size()) checkedItems.set(key, Boolean.TRUE);
            else {
                while (key > checkedItems.size()) checkedItems.add(Boolean.FALSE);
                checkedItems.add(Boolean.TRUE);
            }
