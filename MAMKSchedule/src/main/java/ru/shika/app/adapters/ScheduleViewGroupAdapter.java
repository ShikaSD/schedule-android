...
public class ScheduleViewGroupAdapter extends PagerAdapter {
    ...
    public Object instantiateItem(ViewGroup container, int position) {
        ...
        if (layouts.get(position) != null) layout = layouts.get(position);
        else layout = (RelativeLayout) getItem(container, position);
    ...
    private void updateVisibility(int position) {
        RecyclerView recyclerView = (RecyclerView) (layouts.get(position)).findViewById(R.id.fragment_schedule_list);
        TextView empty = (TextView) (layouts.get(position)).findViewById(R.id.emptySchedule);
        if (lessons.get(position).size() == 0) {
